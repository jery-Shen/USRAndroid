...
public class CRC {
    ...
    public static byte[] getCRC16(byte[] bytes) {
    	byte[] crc_res = new byte[bytes.length+2]; 
    	int crc = calcCrc16(bytes);
    	System.arraycopy(bytes, 0, crc_res, 0, bytes.length);
        crc_res[bytes.length+1] = (byte) (crc>>8);
        crc_res[bytes.length] = (byte) crc;
    	return crc_res;
    }
